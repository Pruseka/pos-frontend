...
const FormModal: React.FC<Props> = ({ item, isEditing, loading, updateRow, addRow }) => {
   ...
   const initialValues = isEditing ? { name: item?.['name'] } : { name: '' }
   const addValidate = { name: isNotEmpty('Name must be filled') }
   const updateValidate = { name: isNotEmpty('Name must be filled') }
